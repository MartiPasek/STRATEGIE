  ...
  _loader("page_render.js", "v1.0.0", function () {
    ...
    function _renderDraftedPlaceholder(mainContent, tab, coreCode, coreId) {
      ...
      if (String(coreCode) === 'hr.dodavatele') {
        mainContent.innerHTML = '<iframe src="/karta-zamestnance?view=dodavatele" title="Agenturní / dodavatelské smlouvy" '
          + 'style="width:100%;height:100%;border:0;display:block;background:#0f141a;"></iframe>';
        return;
      }
      // Nábor (Šárka 21.8.2026): jádro hr.nabor = iframe karty v pohledu náboru (mini-ATS). Jen HR.
      if (String(coreCode) === 'hr.nabor') {
        mainContent.innerHTML = '<iframe src="/karta-zamestnance?view=nabor" title="Nábor" '
          + 'style="width:100%;height:100%;border:0;display:block;background:#0f141a;"></iframe>';
        return;
      }
      ...
      // Pracovní smlouvy (Šárka 24.7.2026): jádro hr.smlouvy = iframe karty v pohledu
      // smluv (řádek = smlouva: pozice, typ, začátek, společnost, oddělení, FTE, denní směna).
      if (String(coreCode) === 'hr.smlouvy') {
        mainContent.innerHTML = '<iframe src="/karta-zamestnance?view=contracts" title="Pracovní smlouvy" '
          + 'style="width:100%;height:100%;border:0;display:block;background:#0f141a;"></iframe>';
        return;
      }
      ...
      if (String(coreCode) === 'hr.podminky') {
        mainContent.innerHTML = '<iframe src="/karta-zamestnance?view=podminky" title="Podmínky pracovníků" '
          + 'style="width:100%;height:100%;border:0;display:block;background:#0f141a;"></iframe>';
        return;
      }
      ...
      // Směrnice (Šárka 1.9.2026): jádro hr.smernice = prohlížeč firemních směrnic
      // (zrcadlo z Centrály, jen čtení). Jen vedení/HR.
      if (String(coreCode) === 'hr.smernice') {
        mainContent.innerHTML = '<iframe src="/karta-zamestnance?view=smernice" title="Směrnice" '
          + 'style="width:100%;height:100%;border:0;display:block;background:#0f141a;"></iframe>';
        return;
      }
