  ...
  function cil(){
    ...
    p.appendChild(el('<div id="cilKill"></div>'));
    ...
    api("GET","/api/v1/erp/app/cil"+q,"").then(function(j){
      var kb=document.getElementById("cilKill");
      if(kb){ kb.innerHTML="";
        if(j&&j.kill){ kb.appendChild(el('<div style="background:#4a1518;border:1px solid #7a2a2f;color:#ffb3b3;border-radius:12px;padding:11px 14px;margin-bottom:10px;font-weight:600;">⛔ Cílový režim je globálně pozastaven (kill switch). Aktivace a obnovení cílů jsou zablokované.</div>')); }
        var _kon=!!(j&&j.kill);
        var tgl=el('<button style="width:100%;margin:0 0 10px;border:0;border-radius:12px;padding:12px;font-size:15px;font-weight:700;color:#fff;background:'+(_kon?"#3fbf6b":"#ef6a6a")+';">'+(_kon?"▶️ Obnovit celý režim":"⛔ Pozastavit celý režim (kill switch)")+'</button>');
        tgl.addEventListener("click",function(){ if(!_kon && !confirm("Pozastavit CELÝ Cílový režim? Zablokuje to aktivaci i obnovení všech cílů.")) return; tgl.disabled=true; api("POST","/api/v1/erp/app/cil/"+(_kon?"unkill":"kill"),{}).then(function(r){ if(r&&r.ok){ cil(); } else { tgl.disabled=false; alert((r&&r.error)||"Chyba"); } }); });
        kb.appendChild(tgl);
      }
