...
2. **Fáze 2:** Dva zápisníky (poznámky + znalosti) + manuální promoce. UI strom s rozbalováním. — **CODE COMPLETE + COMMITNUTO 2026-04-22**.
3. **Fáze 3:** Certainty scoring + auto-promoce nad threshold. User trust rating. Rodičovská role (cross-tenant viditelnost). — **CODE COMPLETE + COMMITNUTO 2026-04-22**.
4. **Fáze 4:** Aktivní učení (Marti se sama ptá) — generator otázek, UI modal s kartami, header badge, noční LLM review textových odpovědí. — **CODE COMPLETE + COMMITNUTO 2026-04-22**.
5. **Fáze 5:** Martiho private diary + todo zápisnice. `record_diary_entry` tool, `list_diary_for_persona` service, UI tile "📖 Diář" pro rodiče. Todo: `mark_todo_done` tool, UI tile "📋 Úkoly" s checkboxy, filter Open/Done/All. — **CODE COMPLETE 2026-04-23**, čeká review + commit.
...
| 6 | Aktivní učení | **Policy se skládá ze 4 částí (6a-6d):**<br>**6a Koho se ptá:** Jen rodiče (`is_marti_parent=True`). Odpovědi rodiče mají plnou váhu (např. certainty +30). Později lze rozšířit na autora + trusted users.<br>**6b UI kanál — hybrid:** (1) **Inline** — Marti se průběžně doptává v běžné konverzaci, kontextově, během dialogu. (2) **Batch konverzace** — speciální konverzace "Otázky od Marti" vždy na první pozici sidebaru (jen pro rodiče). Rodič odpovídá strukturovaně (Ano / Ne / Nejsem si jist / Přeskoč) **bez LLM round-tripu na každou odpověď** — odpovědi jdou přímo do DB + updaty certainty. Periodický batch (např. 1× denně) pak provede LLM syntézu nad sebranými odpověďmi (derivace nových myšlenek, aktualizace souvisejících).<br>**6c Rytmus:** Bez capu na pool. Marti vždy má otázky připravené, rodič volí tempo (někdy 0, někdy 200 za večer). Inline: max 1 otázka per Marti odpověď, aby běžný flow netrpěl. UI musí zvládnout robustně i pool 500+ (priority, paginace, vyhledávání).<br>**6d Priorita:** Automatické řazení **nejnižší jistota + nejčerstvější**. SQL-friendly `ORDER BY certainty ASC, created_at DESC`. Bez extra LLM tokenu. Upgrade na AI-driven ordering až pokud by enterprise-scale pool vyžadoval chytřejší výběr.<br>**6e Schema otázek (Faze 4):** Nová samostatná tabulka `marti_questions` (thought_id FK, question_text, target_user_id, status, answer_choice, answer_text, answered_at, priority_score). Čistá separace od `thoughts` (co Marti ví) a `tasks` (co AI má vykonat).<br>**6f Kdy generovat:** Batch worker `STRATEGIE-QUESTION-GENERATOR` běží 1× za 6h (pattern jako `task_worker` / `email_fetcher`). Navíc **manuální trigger přes tlačítko** v profile / Marti dropdown menu (stejný pattern jako "⟳ Fetch now" u emailu). Rodič si tak může nové otázky vynutit, když je netrpělivý.<br>**6g Formulace otázek:** Plně LLM-generated pro každou myšlenku. Kontext pro LLM: myšlenka (obsah, typ, jistota, věk), autor, zdroj, související myšlenky o stejné entitě, komu se ptá (short_name + vokativ), nedávno položené otázky (avoid redundancy). Styl: přirozená čeština, max 2 věty, odkazy na kontext ("včera jsi říkal..."), s vokativem oslovení. Cena: ~500-800 tokenů per otázka, při 20 otázkách/den ~1-2 Kč.<br>**6h Zpracování odpovědí:** Hybrid — **mechanické úpravy hned** (Ano: +25 certainty, Ne: -40, Nejsem si jist: +0, Přeskoč: status='skipped' bez update). Když certainty přes auto-promote threshold (≥80), myšlenka se povýší do znalostí rovnou. **Textové odpovědi** se uloží k přezkoumání nočnímu LLM batchi, který interpretuje nuance (může upravit content myšlenky, vytvořit korekce, spustit follow-up otázky). Rychlá UX pro rodiče + inteligentní noční syntéza.<br>**6i Výběr kandidátů:** Jednoduché SQL + diverzifikace: `WHERE status='note' AND certainty<70 AND deleted_at IS NULL AND nema_open_otazku ORDER BY certainty ASC, created_at DESC`. **Max 3 otázky per entita per batch run** (rozložení napříč lidmi/projekty). **Max 10 nových otázek per batch run** (celkový strop). LLM-driven výběr odložen na Fázi 5+. | | 2026-04-22 |
