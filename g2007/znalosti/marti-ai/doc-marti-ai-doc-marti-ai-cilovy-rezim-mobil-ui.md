...
## Další gotchy
- **Zpět na všech platformách:** `renderNav` (74) zobrazuje spodní lištu „← Zpět" jen když `stack.length>1 && stg_backbar==='always'` — **je skryta všude** (prohlížeč, Android i iOS). Nativní obrazovka si proto má dát **vlastní viditelné „← Zpět"** (tlačítko volající `back()`) — jinak uživatel na žádné platformě žádné Zpět nevidí. (V `73_zcil.js` helper `_cilBack()` v každé obrazovce.) (Aktualizováno 28. 8. 2026 — dříve platilo jen pro Android, nově platí všude; viz `doc-system-strategie-mobil-spodni-lista-zjednodusena-2026-08-28`.)
