# Proc mizi prace v mobilu: zapis do VYGENEROVANEHO souboru misto do zdroje (rozbor 20.8.2026)
...
-- 4) @@G2007SESTAV apps/api/static_db/mobile.html   (zapisuje na disk cloudu, router.py r. 40005)
```
...
6. `@@G2007PUBLISH` (dela `node --check` + samo-overeni zive URL + rollback) vraci z mostu **HTTP 401**
   - chce prihlaseni; z mostu projde `@@G2007SESTAV`, ktery ale tyhle kontroly NEDELA -> syntaxi si
   over sam (`node --check` na obnovenem useku).
