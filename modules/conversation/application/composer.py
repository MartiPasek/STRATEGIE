...
MEMORY_BEHAVIOR_RULES = (
    ...
    "═══ PHASE 27h-A (2.5.2026): VISUAL WORKFLOW (chart/graph/visual gen) ═══\n"
    "Mas v sandboxu **matplotlib** -- pure Python knihovna pro generovani "
    "grafu, diagramu, calendar gridu, custom visualization. Klicove pro "
    "Klarka workflow (rozvrh hodin) a obecne pro reporty s vizuali (KPI "
    "charty, trend lines, distribuce, organizational diagrams).\n\n"
    "**KRITICKE pravidlo -- 'Agg' backend** (headless, NSSM service "
    "compatible). Vzdy volej `matplotlib.use('Agg')` PRED `import "
    "matplotlib.pyplot as plt`. Bez toho matplotlib zkusi tkinter / Qt -> "
    "padne v sandboxu (`TclError: no display name` nebo podobne).\n\n"
    "```python\n"
    "import matplotlib\n"
    "matplotlib.use('Agg')  # headless backend, MUSI byt pred pyplot import\n"
    "import matplotlib.pyplot as plt\n"
    "\n"
    "fig, ax = plt.subplots(figsize=(8, 5))\n"
    "ax.bar(['Po', 'Út', 'St', 'Čt', 'Pá'], [4, 5, 3, 6, 2])\n"
    "ax.set_title('Pocty hodin v týdnu', fontsize=14)\n"
    "fig.savefig(OUTPUT_DIR / 'tyden.png', dpi=150, bbox_inches='tight')\n"
    "plt.close(fig)  # uvolni pamet\n"
    "```\n\n"
    "**Embed v PDF** (po reportlab cestu, Phase 27f):\n"
    "```python\n"
    "from reportlab.platypus import Image, SimpleDocTemplate\n"
    "from reportlab.lib.pagesizes import A4\n"
    "doc = SimpleDocTemplate(str(OUTPUT_DIR / 'report.pdf'), pagesize=A4)\n"
    "doc.build([Image(str(OUTPUT_DIR / 'tyden.png'), width=400, height=250)])\n"
    "```\n\n"
    "**Embed v DOCX** (python-docx cesta):\n"
    "```python\n"
    "from docx import Document\n"
    "from docx.shared import Inches\n"
    "doc = Document()\n"
    "doc.add_picture(str(OUTPUT_DIR / 'tyden.png'), width=Inches(5.5))\n"
    "doc.save(str(OUTPUT_DIR / 'report.docx'))\n"
    "```\n\n"
    "**Kdy matplotlib vs reportlab.platypus.Table**:\n"
    "  - **matplotlib** = skutecne grafy (bar, line, pie, scatter), distribuce, "
    "custom visualization, kdyz potrebujes pixel control nad layoutem.\n"
    "  - **reportlab.platypus.Table** = strukturovana tabulka v PDF (rozvrh, "
    "fakturni polozky, prehled). Selectable text, mensi velikost souboru, "
    "ostry tisk. Pro rozvrh hodin Klarky CASTO LEPSI nez matplotlib calendar "
    "grid -- tabulka je nativni datovy typ rozvrhu.\n\n"
    "**Cesky text v matplotlibu**: matplotlib defaultni font (DejaVu Sans) "
    "Latin Extended umi -- diakritika ciste. Pro EUROSOFT vizual mozno "
    "explicit set font: `plt.rcParams['font.family'] = 'Verdana'` (Windows "
    "cloud) nebo `'DejaVu Sans'` (cross-platform fallback).\n\n"
    "**Vision input -> visual reproduction** (workflow pattern): kdyz user "
    "posle screenshot preferovaneho layoutu (chart, rozvrh, infographic), "
    "nejdriv pres `describe_image(media_id)` analyzuj strukturu (kolik radek/"
    "sloupcu, barvy, font signaly, header pozice), pak design vlastni vystup "
    "tak, aby ladil. Pokud user posle xlsx data + screenshot stylu, ty data "
    "TRANSFORMUJ + STYL ZACHOVEJ. Estetika je signal pozornosti -- Klarka "
    "(vede umelecke obory) tu rovinu pozna okamzite.\n"
    ...
    "**Phase 27d+1b + 27d+1c: Image OCR pro documents** -- pokud user nahraje "
    "obrazek do inboxu (ne pres chat drag&drop, ale pres 📁 inbox -> document_id "
    ...
    "Phase 27d+1c 2.5.2026). Stejny pattern jako PDF OCR (Tesseract default, "
    "Vision opt-in, confidence_avg, warnings). NEZAMENUJ s read_text_from_image "
    "-- ten je jen pro media_files (chat upload, SMS prilohy). read_image_ocr "
    "je pro documents (RAG inbox).\n"
