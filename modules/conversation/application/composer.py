...
def _build_orchestrate_block(conversation_id: int) -> str | None:
    ...
    try:
        ...
        return (
            ...
            "Tool vraci JSON dict s fields:\n"
            "  {email: {count, top: [{id, from, subject, age, priority}]},\n"
            "   sms:   {count, top: [{id, from, body_preview, age, priority}]},\n"
            "   todo:  {count, top: [{id, content, age, priority}]},\n"
            "   summary: '...'}\n\n"
            "JSON je STRUKTUROVANA DATA jen pro tebe. **NIKDY nekopirujes JSON**\n"
            "ani polozky z neho do sve odpovedi uzivateli. Vytahnes si z nej jen\n"
            "**hodnoty** (pocty, jmena, predmety, casy) a ZABALIS je do plynule\n"
            "prosni vety v cestine.\n\n"
            "KATEGORICKY ZAKAZ:\n"
            "  - NIKDY nepis '---', '===', '[#8]', 'p=100', 'priority:', fieldy\n"
            "    jako 'subject:', 'top:', 'body_preview:'. To je JSON syntaxe,\n"
            "    ne lidska komunikace.\n"
            ...
            "UKAZKA -- JSON vstup:\n"
            "  {'email': {'count': 3, 'top': [{'id': 4, 'from': 'Marti', 'subject': 'Pro tebe...', 'age': 'vcera'}]},\n"
            "   'sms': {'count': 0, 'top': []},\n"
            "   'todo': {'count': 2, 'top': [{'content': 'Smazat testovaci usery'}]}}\n\n"
