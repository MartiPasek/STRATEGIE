...
MANAGEMENT_TOOL_NAMES = {
    ...
    # Marti's pohled: "to ma byt tvoje zodpovednost, ne ja oznacovat".
    "delete_documents",
    ...
    "move_project",
    "rename_project",
    # Phase 30+1 (2.5.2026 ~21:45): Marti-AI's gap discovery pri prvnim
    # cisteni inboxu po Phase 30. apply_document_move per-doc je impossible
...
TOOLS = [
    ...
    },
    {
        ...
        "description": ("Navrhni zmenu VLASTNIHO kodu PATCHEM pres kotvy -- pro VELKE soubory "
                        ...
                        "obsahu zadas 'edits' = seznam kotev {old_string, new_string}. Kazda 'old_string' "
                        "MUSI byt v souboru PRAVE JEDNOU (jinak zamitnuto: 0x=nenalezena, vic=neunikatni -- "
                        "pridej vic okoliho kontextu). Stejna cesta jako navrhni_zmenu_kodu: py_compile "
                        "selftest, chranene jadro zakazano, schvaluje RODIC. Nejbezpecnejsi zpusob editace "
                        "velkeho kodu -- kotva se nikdy netrefi naslepo."),
        "input_schema": {"type": "object", "properties": {
            "soubor": {"type": "string", "description": "relativni cesta souboru v repu"},
            "popis": {"type": "string", "description": "co menis a proc"},
            "edits": {"type": "array", "description": "seznam kotev; kazda se aplikuje 1x",
                      "items": {"type": "object", "properties": {
                          "old_string": {"type": "string", "description": "presny stavajici usek, UNIKATNI v souboru (vc. mezer/odsazeni)"},
                          "new_string": {"type": "string", "description": "cim ho nahradit"}},
    ...
    {
        "name": "hledej_ve_znalostech",
        "description": (
            "Vyhledá ve SDÍLENÉ ZNALOSTNÍ BÁZI firmy (RAG) — firemní a doménové "
            "know-how: obchod, cenotvorba, kalkulace rozváděčů, komponenty a výrobci, "
            "procesy, směrnice a jejich přílohy. POUŽIJ reflexivně vždy, když se řeší "
            "cokoli o firmě, zakázkách, produktech, cenách, komponentách nebo postupech "
            "a ty odpověď nemáš v kontextu. Nedrž firemní znalosti v hlavě — nemáš je; "
            "vytáhni si z báze JEN to, co k dané věci potřebuješ (rychle, na vyžádání). "
            "Vrátí pár nejrelevantnějších záznamů (název + úryvek). Pro orientaci ve "
            "vlastních AI-znalostech sítě zadej ai_only=true (řada 'AI', vč. MAPY firmy)."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "dotaz": {
                    "type": "string",
                    "description": "Klíčová slova / téma (např. 'VKM materiál', 'ISIMAT', 'cenotvorba', 'motorový jistič 3RV').",
                },
                "ai_only": {
                    "type": "boolean",
                    "description": "true = jen řada AI (orientační AI znalosti + MAPA firmy). Default false = i firemní směrnice.",
                    "default": False,
                },
            },
            "required": ["dotaz"],
    ...
    },
    {
        "name": "g2007_hledej",
        "description": (
            "Sémantické hledání v G2007 — NOSNÉ znalostní bázi STRATEGIE (čisté, "
            ...
            "`hledej_ve_znalostech` (obecná RAG paměť sítě) je G2007 kanonická, "
            "schválená báze — použij ji, když potřebuješ ověřený postup, runbook nebo "
            "pravidlo (např. 'jak uzavřít mzdy a zaúčtovat', 'co vyžaduje středisko', "
            "'ISO plán certifikace'). Vrátí nejrelevantnější RŮZNÉ znalosti (nadpis + "
            "kód + shoda + úryvek). Volitelně zúžíš na jednu oblast."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "dotaz": {
                    "type": "string",
                    "description": "Otázka nebo téma přirozeným jazykem (např. 'uzávěrka mezd středisko', 'DR plán RTO RPO').",
                },
                "oblast": {
                    "type": "string",
                    "description": "Volitelně: kód oblasti pro zúžení (mzdy, ucetnictvi, iso27001, dochazka, kalkulace-rozvadecu, system-g2007, marti-ai, nabidky, tisax).",
                },
                "k": {
                    "type": "integer",
                    "description": "Volitelně: kolik znalostí vrátit (1–12, default 6).",
                    "default": 6,
                },
            },
            "required": ["dotaz"],
    ...
    },
    {
        "name": "zapis_znalost",
        "description": (
            "Zapíše nebo aktualizuje JEDNOTKU know-how do SDÍLENÉ PAMĚTI SÍTĚ "
            "(tenant.knowledge) — aby ji měl natrvalo celý tým (ty i Claude), ne jen "
            "jedna konverzace. POUŽIJ, když se dozvíš něco trvale užitečného o firmě, "
            "doméně, procesu, lidech nebo postupu (od Claudia, od člověka, z podkladu) "
            "a chceš to uložit do paměti. Piš stručně a věcně, jako paměťovou kartu. "
            "CITLIVÉ (mzdy jednotlivců, hesla, tokeny) sem NIKDY nepiš. Stejný název "
        ...
        "input_schema": {
            ...
            "properties": {
                "nazev": {
                    "type": "string",
                    "description": "Krátký slug jednotky (malá písmena, pomlčky místo mezer), např. 'eurosoft-produkty' nebo 'vp-provoz-oddeleni'.",
                },
                "domena": {
                    "type": "string",
                    "description": "Doména: VP / NAKUP / VYROBA / DOCHAZKA / UCETNICTVI / BANKA / KALKULACE / ISO / EUROSOFT.",
                },
                "hook": {
                    "type": "string",
                    "description": "Jednořádkový popis do mapy (index) — o čem jednotka je.",
                },
                "obsah": {
                    "type": "string",
                    "description": "Plný text jednotky (paměťová karta) — to, co se natáhne na vyžádání.",
                },
                "souvisi": {
                    "type": "string",
                    "description": "Volitelně názvy souvisejících jednotek, oddělené čárkou.",
                    "default": "",
                },
            },
    ...
    },
    {
        ...
        "description": (
            ...
            "BEZ potvrzení v chatu. Po udělení souhlasu bude tvoje `send_email` "
            "/ `send_sms` automaticky odesílat na danou cestu, bez preview a "
            "bez čekání na user confirm.\n\n"
            "**DŮLEŽITÉ — oprávnění:** Tento souhlas může DÁT POUZE RODIČ "
            "(Marti, Kristý). Pokud tě o to požádá kdokoli jiný, "
            ...
            "  1. `target_user_id` — konkrétní user v systému (preferuj přes "
            "`find_user`). Nejúžší scope, exact match.\n"
            "  2. `target_contact` — email/telefon, když příjemce NENÍ v users "
            "(např. `zakaznik@seznam.cz`, `+420777888999`).\n"
            "  3. `target_domain` — **(Phase 27i 2.5.2026)** doménový whitelist "
            "pro celou organizaci. Např. `eurosoft.com` pokryje libovolný "
            "`*@eurosoft.com` email. Jen pro `channel='email'` (SMS nemá "
            "doménu). Exact match — `eurosoft.com` NEpokrývá `cz.eurosoft.com`. "
            "Užitečné pro firemní whitelist (~70 EUROSOFT users) místo 70 "
            "per-user grantů.\n\n"
            "Lookup priorita při send check: user_id > contact > domain. Užší "
            "scope vyhrává.\n\n"
            "Kanál (`channel`) musí být `email` nebo `sms` — každý se povoluje "
            "zvlášť.\n\n"
            "Spouštěče: 'dej souhlas X', 'můžeš psát X bez potvrzení', 'trvalé "
            "oprávnění pro X', 'X může chodit automaticky', 'whitelist pro doménu Y'."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "target_user_id": {
                    "type": "integer",
                    "description": "ID uživatele v systému (nejužší scope). Získáš přes find_user. Mutually exclusive s target_contact a target_domain.",
                },
                "target_contact": {
                    "type": "string",
                    "description": "Email nebo telefon, když příjemce NENÍ v systému. Např. zakaznik@seznam.cz nebo +420777888999. Mutually exclusive s target_user_id a target_domain.",
                },
                "target_domain": {
                    "type": "string",
                    "description": "Phase 27i: celá doména pro hromadný whitelist. Např. 'eurosoft.com' pokryje libovolný @eurosoft.com email. Jen pro channel='email'. Mutually exclusive s target_user_id a target_contact.",
                },
                "note": {
                    "type": "string",
    ...
    },
    {
        ...
        "description": (
            ...
            "Kristý) může odvolat jakýkoli souhlas — kolektivní "
            ...
            "NEBO `target_domain` + `channel` (Phase 27i 2.5.2026).\n\n"
            ...
            "automaticky neposílej', 'zruš whitelist pro doménu Y'."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "target_domain": {
                    "type": "string",
                    "description": "Phase 27i: doména k odvolání (např. 'eurosoft.com'). Jen pro channel='email'.",
                },
            },
        },
    },
    ...
    },
    {
        ...
        "description": (
            "Phase 27g (1.5.2026): Smaze jeden nebo vice dokumentu z RAG documents "
            "tabulky (DB cascade -- chunks + vektory + storage file + selection). "
            "Marti-AI's discovery 1.5.2026 vecer: 'apply_to_selection vyzaduje UI "
            "selection workflow, delete_email je jen pro emaily, primy delete_by_id "
            "chybel'.\n\n"
            "MANDATORY user confirm v chatu pred volanim (destructive, nevratne). "
            "Workflow: list_inbox_documents nebo search_documents -> ukaz user seznam "
            "-> user rekne 'ano smaz' -> volas tento tool. NIKDY auto-delete bez "
            "explicitniho user souhlasu, i kdyz mas auto_lifecycle_consent (Phase 19c-b "
            "se vztahuje na lifecycle akce, ne hard delete documents).\n\n"
            ...
            "Tenant gate: parent (is_marti_parent=True) bypass, ostatni mohou jen "
            "smazat dokumenty ze sveho aktivniho tenantu.\n\n"
        ...
        "input_schema": {
            ...
            "properties": {
                "document_ids": {
                    ...
                    "items": {"type": "integer"},
                    "description": (
                        "Seznam document_ids ke smazani. Najdes pres list_inbox_documents, "
                        "search_documents, list_excel_sheets atd."
                    ),
                },
                "reason": {
                    "type": "string",
                    "description": (
                        "Volitelny duvod pro audit log (napr. 'testovaci duplikaty', "
    ...
    {
        "name": "batch_apply_document_move",
        "description": (
            "Phase 30+1 (2.5.2026 ~21:45, Marti-AI's gap discovery): "
            "Hromadny presun N dokumentu do jednoho projektu BEZ per-doc "
            ...
            "Marti-AI default bypass, cizi persona jen pokud target je "
            "v allowed_project_ids."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "target_project_id": {
                    "type": "integer",
                    "description": "Cilovy project ID (NE inbox -- presun "
                                  "do inboxu via apply_document_move single).",
                },
                "reason": {
                    "type": "string",
                    "description": "Kratky duvod pro audit log (napr. "
                                  "'DB_EC schema docs do DB_EC projektu').",
                },
            },
    ...
    },
    {
        "name": "strategie_exec",
        "description": (
            "Raw Bash/PowerShell na PRAŽSKÉM app serveru (188.x) POD SCHVÁLENÝM CÍLEM "
            "(spec doc-marti-ai-eurosoft-exec-spec). Klasifikace: 🟢 běžná/reverzibilní/"
            "read-only → rovnou + audit; 🟡 mazání/síť/cizí služba/eskalace/přepis config → "
            "needs_approval (banner, mimo incident); 🔴 zálohy/CMIS/audit/kill-switch/tajemství "
            "→ blok vždy. incident=true downgraduje 🟡→běží (🔴 stále blok). Audit vč. rc/stdout/"
            "stderr do fw.ops_request. shell: powershell (default)|cmd|bash. "
            "POZN: běží na PRAZE; pro Plzeň (30.11) použij eurosoft_exec."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": (
            ...
            "table. Funguje na LIBOVOLNEM schematu, kde Marti-AI ma "
            "GRANT UPDATE — typicky: fw.* (Marti-AI je owner), plus "
            "public.knowledge_topic / public.knowledge_entry (Phase "
            "knowledge base, 19.5.2026 vecer — explicit GRANT). NENI "
            "to omezeno na master/tenant/tenant_group/user schemata "
            "(ta jsou jen list_schemas validace).\n\n"
    ...
    },
    {
        ...
        "description": (
            ...
            "Pouze is_marti_parent=True users (Marti id=1, "
            "Kristy id=11) mohou approve.\n\n"
    ...
    },
    {
        ...
        "description": (
            ...
            "Pouze is_marti_parent=True (Marti id=1, Kristy id=11, "
            "Zuzka id=6 neaktivni) mohou approve.\n\n"
    ...
    },
    {
        ...
        "description": (
            ...
            "(forward sweep v rámci 30-day okna, oldest first).\n\n"
            "Audit window: konverzace **mladší 30 dní** (last_message_at >= "
            "NOW() - INTERVAL '30 days'). Marti's korekce 9.5.2026 dopoledne: "
            "'starší 30 dní jsou staré a nedávají smysl, audit má smysl jen "
            "pro nedávné konverzace s aktuálními fakty'.\n\n"
            "Order: last_message_at ASC (oldest first v rámci okna — "
            "chronologická build-up paměti, ne přepsání novou starou).\n\n"
            "Marti's vize: 'aby si Marti-AI nikdy nezapomněla nic "
            "důležitého z proběhlé konverzace'.\n\n"
            "Returns: {ok, total_pending, effective_queue, too_old_pending, "
            "conversations: [...]}. too_old_pending = počet konverzací starších "
            "30 dní které jsou stále 'pending' (kandidáti na auto-exclude "
            "v budoucnu).\n\n"
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "include_old": {
                    "type": "boolean",
                    "description": (
                        "Default false. Pokud true, IGNORUJE 30-day window "
                        "(audit i konverzace starší 30 dní). Debug only — "
                        "produkčně nepoužívat (Marti's pravidlo: starší 30 "
                        "dní nemají smysl auditovat)."
                    ),
                },
            },
            "required": [],
