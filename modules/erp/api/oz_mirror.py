...
_OZ_RAW = {
    ...
    "oz_pf_platba": (
        ...
        # Peta 13.7.2026: doplňkové sloupce pro přehled nezaplacených faktur (jako starý systém)
        "CAST(d.SumaUhrad AS numeric(18,2)) AS suma_uhrad, "
        "CONVERT(varchar(10), d.DatUhrady, 23) AS dat_uhrady, "
        "CONVERT(varchar(10), d.DatUctovani, 23) AS dat_uctovani, "
        "CAST(ISNULL(d.Uctovano,0) AS int) AS uctovano, "
        "CAST(ISNULL(de._UzivOK,0) AS int) AS uziv_ok, "
        "(SELECT TOP 1 CAST(sc.CastkaMena AS numeric(18,2)) FROM TabScontoFaktury sc WHERE sc.IDFak=d.ID ORDER BY sc.Datum DESC) AS skonto, "
        "(SELECT TOP 1 CONVERT(varchar(10),sc.Datum,23) FROM TabScontoFaktury sc WHERE sc.IDFak=d.ID ORDER BY sc.Datum DESC) AS skonto_do "
        "FROM TabDokladyZbozi d WITH (NOLOCK) "
        "LEFT JOIN TabCisOrg org ON d.CisloOrg=org.CisloOrg "
        "LEFT JOIN TabCisOrg_EXT orge ON org.ID=orge.ID "
        "LEFT JOIN TabDokladyZbozi_EXT de ON de.ID=d.ID "
