...
_OZ_RAW = {
    ...
    "oz_platak_tuz": (
        ...
        "UNION ALL "
        "SELECT pt.ID AS id, 'ES' AS firma, pt.RealizaceExport AS realizace_export, "
        "CONVERT(varchar(10),pt.DatumVystaveni,23) AS datum_vystaveni, "
        "CONVERT(varchar(10),pt.DatumSplatnosti,23) AS datum_splatnosti, "
        "CONVERT(varchar(19),pt.DatPorizeni,120) AS dat_porizeni, "
        "bs.NazevBankSpoj AS odkud, "
    ...
    "oz_platak_zahr": (
        ...
        "UNION ALL "
        "SELECT pz.ID AS id, 'ES' AS firma, pz.RealizaceExport AS realizace_export, "
        "CONVERT(varchar(10),pz.DatumVystaveni,23) AS datum_vystaveni, "
        "CONVERT(varchar(10),pz.DatumSplatnosti,23) AS datum_splatnosti, "
        "CONVERT(varchar(19),pz.DatPorizeni,120) AS dat_porizeni, "
        "plat.NazevBankSpoj AS odkud, prij.NazevBankSpoj AS komu, "
        ...
        "pz.Poplatky AS poplatky, pz.Priorita AS priorita, pz.IDObdobi AS id_obdobi, "
        "pz.IDBankSpojeniPlatce AS id_bank_spojeni, "
        "CAST((SELECT STRING_AGG(CONVERT(nvarchar(20),r._PorCisFak),', ') FROM [DB_IS].dbo.TabPlatZahrDetail o "
        "LEFT OUTER JOIN [DB_IS].dbo.TabPlatZahrDetail_EXT r ON r.ID=o.ID WHERE o.IDHlavaPPZ=pz.ID) AS nvarchar(4000)) AS seznam_faktur, "
        "pze._PathPayFile AS path_pay_file "
        "FROM [DB_IS].dbo.TabPlatZahr pz WITH (NOLOCK) "
        "LEFT OUTER JOIN [DB_IS].dbo.TabPlatZahr_EXT pze ON pz.ID=pze.ID "
        "LEFT OUTER JOIN [DB_IS].dbo.TabPenezniUstavy ubu ON pz.IDBankUstavuPlatce=ubu.ID "
        "LEFT OUTER JOIN [DB_IS].dbo.TabBankSpojeni plat ON pz.IDBankSpojeniPlatce=plat.ID "
        "LEFT OUTER JOIN [DB_IS].dbo.TabPenezniUstavy pbu ON pz.IDBankUstavuPrijemce=pbu.ID "
        "LEFT OUTER JOIN [DB_IS].dbo.TabBankSpojeni prij ON pz.IDBankSpojeniPrijemce=prij.ID "
        "WHERE YEAR(pz.DatPorizeni) IN (2025,2026)"
    ),
}
