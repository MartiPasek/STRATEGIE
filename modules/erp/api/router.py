...
def fw_core_page_spec(core_id: int, req: Request) -> JSONResponse:
    ...
    try:
        ...
        # Phase 38.4 Krok 5.R-C (18.5.2026, Marti's "self-healing column
        # registry"): pokud root je grid (list_root/grid_modern/list),
        # ensure grid_master + sync comp_grid_column vs dataset fields.
        columns_list = None
        if root_row:
            type_code = root_row.get("type_code") or ""
            if type_code in ("list_root", "grid_modern", "list"):
                ds_id = root_row.get("data_source_id")
                ds_code = root_row.get("data_source_code")
                # Sample 1 row z data_source pro field names (truth source)
                dataset_fields = []
                if ds_code:
                    try:
                        sample = ds_runner.run_data_source(
                            ds, code=ds_code,
                            raw_params={"limit": "1"},
                            kind="select",
                        )
                        ...
                        if sample_rows:
                            dataset_fields = list(sample_rows[0].keys())
                    except Exception as exc:
                        import logging as _log_psc
                        _log_psc.getLogger(__name__).warning(
                            "page-spec dataset sample failed for ds_code=%s: %s",
                            ds_code, exc,
                        )
                if dataset_fields:
                    master = _ensure_grid_master(ds, core_id, ds_id, ds_code)
                    if master:
                        columns_list = _sync_grid_columns(
                            ds, master["id"], dataset_fields,
                        )
