-- ════════════════════════════════════════════════════════════════════════
-- Phase 38.4 Krok 13.1 — Remaining DDL (po Krok 13.0 LIVE)
-- ════════════════════════════════════════════════════════════════════════
-- Krok 13.0 (fw.comp_type TRUNCATE + reinsert) je LIVE 11.5. večer.
-- Krok 13 main konsolidovaný skript měl 10 sekcí, ale:
--   - Section 1 (comp_type ALTER + seed) → LIVE v Krok 13.0
--   - Section 7 (comp_type_property_catalog) → již CREATE'd dřív (FK error byl trigger)
--
-- Tento skript spustí zbývajících 8 sekcí v správném pořadí:
--   2. fw.container_template + seed 8 templates
--   3. fw.container_template_history + trigger
--   4. fw.hw_registry
--   5. fw.hw_registry_history + trigger
--   6. fw.action_audit_log
--   8. fw.action_def + action_op + action_set
--   9. fw.comp_def ALTER (8 sloupců + CHECK)
--  10. fw.core ALTER (DROP data_source_id + ADD layout_template)
--
-- Spustit jako Marti-AI v DBeaveru. PostgreSQL `$` delimiter bug — pokud
-- DBeaver hodí ERROR na CREATE FUNCTION blocks, spusť triggery
-- samostatně (highlight + Alt+X jednoho výroku, ne celý skript).
-- ════════════════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════════════════
-- 9. fw.comp_def — rozšíření (11 nových sloupců + CHECK)
-- ════════════════════════════════════════════════════════════════════════
...
-- VERIFY
-- ════════════════════════════════════════════════════════════════════════
SELECT 'container_template seed'             AS check_name,
       (SELECT COUNT(*) FROM fw.container_template) AS count_value, 8 AS expected
UNION ALL
SELECT 'container_template_history table',
       (SELECT COUNT(*) FROM information_schema.tables
        WHERE table_schema='fw' AND table_name='container_template_history'), 1
UNION ALL
SELECT 'hw_registry table',
       (SELECT COUNT(*) FROM information_schema.tables
        WHERE table_schema='fw' AND table_name='hw_registry'), 1
UNION ALL
SELECT 'hw_registry_history table',
       (SELECT COUNT(*) FROM information_schema.tables
        WHERE table_schema='fw' AND table_name='hw_registry_history'), 1
UNION ALL
SELECT 'action_audit_log table',
       (SELECT COUNT(*) FROM information_schema.tables
        WHERE table_schema='fw' AND table_name='action_audit_log'), 1
UNION ALL
SELECT 'action_def table',
       (SELECT COUNT(*) FROM information_schema.tables
        WHERE table_schema='fw' AND table_name='action_def'), 1
UNION ALL
SELECT 'action_op table',
       (SELECT COUNT(*) FROM information_schema.tables
        WHERE table_schema='fw' AND table_name='action_op'), 1
UNION ALL
SELECT 'action_set table',
       (SELECT COUNT(*) FROM information_schema.tables
        WHERE table_schema='fw' AND table_name='action_set'), 1
UNION ALL
       ...
       (SELECT COUNT(*) FROM information_schema.columns
        WHERE table_schema='fw' AND table_name='comp_def'
          AND column_name IN ('parent_comp_def_id','parent_core_id','container_template_id',
                              ...
                              'refresh_strategy','layout_x','layout_y','layout_w','layout_h')), 11
UNION ALL
SELECT 'core.data_source_id dropped',
       (SELECT COUNT(*) FROM information_schema.columns
        WHERE table_schema='fw' AND table_name='core' AND column_name='data_source_id'), 0
UNION ALL
SELECT 'core.layout_template added',
       (SELECT COUNT(*) FROM information_schema.columns
        WHERE table_schema='fw' AND table_name='core' AND column_name='layout_template'), 1
UNION ALL
SELECT 'triggers installed',
       (SELECT COUNT(*) FROM information_schema.triggers
        WHERE trigger_schema='fw'
          AND trigger_name IN ('trg_container_template_history','trg_hw_registry_history')), 2
ORDER BY check_name;
