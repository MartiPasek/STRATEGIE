-- ============================================================================
-- Krok F — UPDATE orchestrator: existence check edit jádra
-- ============================================================================
-- Marti's algoritmus (25.5.2026 vecer):
--   1. parse SANDBOX_CONTEXT env (z F2 backend inject)
--   2. SELECT fw.data_source_op WHERE data_source_id=:rowId
--      AND operation_kind IN ('edit', 'insert')
--   3. Pokud row exists + core_id IS NOT NULL → check fw.core existence
--   4. Pokud row exists + core_id IS NULL → "není sparovaný"
--   5. Pokud žádný row → "žádný op pro tento data_source"
--
-- Drz PoC scope: rowId = data_source_id (master Data Sources grid only).
--
-- Marti spusti v DBeaveru jako Marti-AI session (db_owner fw).
-- ============================================================================

UPDATE fw.executable_artifact
SET source = $orch$# Vytvořit edit jádro — Krok F: existence check
#
# Marti's algoritmus (25.5.2026 vecer):
#   1. parse SANDBOX_CONTEXT env (z F2 backend inject)
...
print("=" * 64)
$orch$,
    description = 'Krok F: existence check edit jádra. Parse SANDBOX_CONTEXT → SELECT fw.data_source_op (kind edit/insert) → check fw.core existence. PoC scope: rowId = data_source.id (master Data Sources grid).',
    updated_at = NOW()
...
WHERE code = 'vytvor_edit_jadro';
-- Expected: 1 row, source_length_chars ~3500, updated_at = teď
