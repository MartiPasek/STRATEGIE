-- ============================================================================
-- Krok F hotfix #2 — Use coreId (NOT rowId)
-- ============================================================================
-- Marti's catch (25.5.2026 vecer):
--   "Ty potrebujes jen core_id (grid) v tomto pripade 40.
--    Nepotrebujes vedet ID zaznamu rowId. To s tim nema nic spolecneho."
--
-- ROOT CAUSE moje chyba:
--   Pouzival jsem rowId (= ID konkretni row gridu, napr. user.id=2 pro
--   Marti-AI v STRATEGIE Users gridu). To je ID DATA recordu, ne struktury.
--
-- SPRAVNY ALGORITHM:
--   1. coreId (= fw.core.id pro grid, napr. 40 = STRATEGIE Users)
--   2. SELECT data_source_id FROM fw.comp_def WHERE parent_core_id = coreId
--      AND data_source_id IS NOT NULL (= grid root komponenta)
--   3. SELECT data_source_op WHERE data_source_id = X
--      AND operation_kind IN ('edit', 'insert')
--   4. Pokud op + core_id IS NOT NULL → check fw.core existence
--   5. Pokud op + core_id IS NULL → "spárování chybí"
--   6. Pokud žádný op → "edit jádro neexistuje, krok 2 (další iterace)"
--
-- rowId zatim nepotrebujeme — to az pri otevreni samotneho edit jadra
-- (kdy uzivatel rekne "editovat tento konkretni zaznam").
--
-- Marti spusti v DBeaveru jako Marti-AI session (db_owner fw).
-- ============================================================================

UPDATE fw.executable_artifact
SET source = $orch$# Vytvořit edit jádro — Krok F hotfix #2: use coreId (NOT rowId)
#
# Marti's algoritmus (25.5.2026 vecer, korigovany):
#   1. parse SANDBOX_CONTEXT env → ziskej coreId (= fw.core.id)
#   2. find grid root comp_def (parent_core_id=coreId, data_source_id NOT NULL)
#   3. ziskej data_source_id z grid root
#   4. SELECT data_source_op WHERE data_source_id=X AND kind IN ('edit','insert')
#   5. Pokud op + core_id NOT NULL → check fw.core existence (EXISTUJE / ORPHAN)
#   6. Pokud op + core_id NULL → "SPÁROVÁNÍ CHYBÍ"
#   7. Pokud žádný op → "EDIT JÁDRO NEEXISTUJE (krok 2 vytvoří)"
import os
import json
import psycopg2
...
def main():
    print("=" * 64)
    print("✨ Vytvořit edit jádro — Krok F: existence check (use coreId)")
    print("=" * 64)
    print()
    ...
    try:
        ...
        # ── Step 3: Find grid root comp_def → data_source_id ───────────────
        # Grid root = comp_def s parent_core_id=coreId a data_source_id NOT NULL.
        # Pokud jich je vic (napr. master + detail), vezmeme prvni (ORDER BY id).
        cur.execute("""
            SELECT id, code, label, data_source_id
            FROM fw.comp_def
        ...
        for gr in grid_roots:
            gr_id, gr_code, gr_label, gr_ds_id = gr
            print(f"  comp_def #{gr_id} | code={gr_code or 'NULL'} | "
                  f"label={gr_label or 'NULL'} | data_source_id={gr_ds_id}")
        print()
        ...
        print(f"Primary grid root: comp_def #{primary_root[0]}")
        print(f"  → data_source_id = {primary_ds_id}")
        print()

        # ── Step 4: Lookup data_source + ops ───────────────────────────────
        cur.execute("""
...
$orch$,
    description = 'Krok F hotfix #2 (use coreId NE rowId): existence check edit jádra. Chain: coreId → fw.comp_def (grid root, data_source_id NOT NULL) → fw.data_source_op (kind edit/insert) → fw.core check. Universal — funguje na jakemkoli gridu, ne jen Data Sources.',
    updated_at = NOW()
...
WHERE code = 'vytvor_edit_jadro';
-- Expected: 1 row, source_length_chars ~5000, updated_at = teď
