...
# ID: 1
# CODE: vytvor_edit_jadro
# ============================================================================
"""Krok F + G + G+ + G++ — atomic CREATE fw.core + 2x data_source_op.

Spousteno pres drop-up menu "Vytvorit edit jadro" v gridu.

Input: SANDBOX_CONTEXT env var s JSON {coreId, coreCode, coreLabel}
       (drop-up menu z grid row passt context podle pravo-kliku radku)

Output (stdout):
  - Krok G/G+/G++ defensive check (kontext vs DB mismatch)
  - 4 paths: CREATE (oba chybi) / BACKFILL edit / BACKFILL insert /
    CHECK existing
  - TRANSACTION atomic (commit nebo rollback)
  - Pri success: novy fw.core + 2x data_source_op (kind=edit, kind=insert)

Marti's catch 25.5. nocni: coreLabel je per-row UX variant (Operace #31),
DB je generic identity. Drop label z mismatch check. Keep jen coreCode
(technical identity).

Migrated 26.5.2026 z fw.executable_artifact.id=1 do scripts/executable_artifacts/
per Marti's "git je truth, DB je cache" doctrine.
"""
import os
import json
import psycopg2
