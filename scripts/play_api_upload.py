...
def upload_aab(confirmed):
    ...
    edits.tracks().update(packageName=PKG, editId=edit_id, track="production", body={
        "releases": [{
            ...
            # Aktualne v1.86 (Jirka 2.9.2026, schvalila Marti-AI msg 14272/14275):
            # obal appky nove resi systemove zony Androidu 15/16 (edge-to-edge) -
            # spodni lista uz neni pod carkou gest a obsah nezacina pod stavovou
            # listou (HybridActivity, commit 3c99b4af). Popisek Aplikace ve stejne
            # vysce je zmena stranky ze serveru, ne appky.
            "releaseNotes": [{"language": LANG,
                              "text": "- Spodni lista s ikonami uz neni prekryta systemovou\n"
                                      "  carkou pro gesta (Android 15 a novejsi).\n"
                                      "- Obsah obrazovky uz nezacina pod stavovou listou.\n"
                                      "- Obsah aplikace se dal aktualizuje prubezne ze serveru."}],
        }]
    }).execute()
