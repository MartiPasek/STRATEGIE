...
def solve(group,tbusy,rbusy,tsec,tdays_prior,hint=None):
    ...
    # Varianta D: Vlková (UNS6G) jen JEDNOU do večera = max 1 den s výukou v 9.-10. h
    if VARIANT=="D":
        _ev=[]
        for _d in range(1,6):
...
# replikuj výpočet kandidátů jako v solve(), zjisti units s 0 kandidáty
tbusy=set(tbusy0); rbusy=set()
zero=[]; tot_units=len(units); tot_cap=0
for u in units:
    cs=0
    for d in range(1,6):
        for h in range(1,u["cap"]-u["L"]+2):
            ...
            for r in rl:
                cs+=1
    tot_cap+=cs
    if cs==0: zero.append((_lbl(u["trid"]),u["pnaz"][:26],u["teachers"][0],"L"+str(u["L"]),"rooms="+",".join(u["rooms"])))
print("units:",tot_units,"| s 0 kandidáty:",len(zero),"| prům. kandidátů/unit:",round(tot_cap/tot_units,1))
for z in zero[:25]: print("   ✗",z)
