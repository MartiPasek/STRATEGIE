...
def solve(group,tbusy,rbusy,tsec,tdays_prior,hint=None):
    ...
    for ui,u in enumerate(group):
        ...
        for k,y in enumerate(yv[ui]):
            wk=w
            if VARIANT=="C":
                _d,_h,_r=cand[ui][k]; wk=w-7*(_h-1)   # C: zaplňuj shora (dřívější hodina = vyšší váha)
            terms.append(wk*y)
...
def gaps(jf):
    # obsazené hodiny po (trida_label, den): jazyky+TV (cbusy) + odborné z jf
    occ=defaultdict(set)
    for (c,d,h) in cbusy:  # cbusy = jazyky+tv pro GDMI třídy (raw kód)
        occ[(c,d)].add(h)
    for c in json.load(open(jf))["cells"]:
        for i in range(c["L"]): occ[(c["trid"],c["d"])].add(c["h1"]+i)
    tot_gap=0; tot_span=0; ndays=0
    for (cc,d),hs in occ.items():
        if len(hs)<2: continue
        lo,hi=min(hs),max(hs)
        span=hi-lo+1; filled=len(hs); gap=span-filled
        # povol 1 hodinu oběd (gap 1 = ok)
        tot_gap+=max(0,gap); tot_span+=span; ndays+=1
    return tot_gap, ndays
ga,na=gaps("gen_pred_v19_A.json")
gc,nc=gaps("gen_pred_v19_C.json")
print(f"A: mezer celkem {ga} přes {na} třída-dnů  (prům {ga/na:.2f}/den)")
print(f"C: mezer celkem {gc} přes {nc} třída-dnů  (prům {gc/nc:.2f}/den)")
