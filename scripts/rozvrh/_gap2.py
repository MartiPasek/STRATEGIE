...
def solve(group,tbusy,rbusy,tsec,tdays_prior,hint=None):
    ...
    for ui,u in enumerate(group):
        ...
        for k,y in enumerate(yv[ui]):
            wk=w
            if VARIANT=="C":
                _d,_h,_r=cand[ui][k]; wk=w-28*(_h-1)   # C: zaplňuj shora (dřívější hodina = vyšší váha)
            terms.append(wk*y)
...
def m(jf):
    occ=defaultdict(set)
    for (c,d,h) in cbusy: occ[(c,d)].add(h)
    for c in json.load(open(jf))["cells"]:
        for i in range(c["L"]): occ[(c["trid"],c["d"])].add(c["h1"]+i)
    excess=0; lastsum=0; nd=0; bigwin=0
    for (cc,d),hs in occ.items():
        if len(hs)<2: continue
        lo,hi=min(hs),max(hs); gap=(hi-lo+1)-len(hs)
        excess+=max(0,gap-1)      # mezery nad rámec 1 obědové
        if gap>1: bigwin+=1
        lastsum+=hi; nd+=1
    return excess,bigwin,lastsum/nd,nd
for f,lab in [("gen_pred_v19_A.json","A"),("gen_pred_v19_C.json","C")]:
    e,b,la,nd=m(f); print(f"{lab}: mezery nad oběd {e} | dnů s velkým oknem(>1) {b} | prům. konec dne {la:.2f}")
