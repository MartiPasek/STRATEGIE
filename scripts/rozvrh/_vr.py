...
def verify(jf):
    cells=json.load(open(jf))["cells"]
    bad=[]; tocc=defaultdict(list); rocc=defaultdict(list); cocc=defaultdict(list)
    WH={}
    ...
    for ln in open("raw_skup.txt"):
        ...
        if ln:
            t,p,s,u,h=ln.split("|")
            if t in GDMI and s!=WH.get(t): LANES[t].add(s)
    def subs(t,sk):
        if sk==WH.get(t): return [(t,ln) for ln in (LANES.get(t) or {sk})]
        return [(t,sk)]
    for c in cells:
        # učebna v povolené sadě dle dokumentu?
        if c.get("room"):
            allowed=room_doc(c["trid"],c["pnaz"])
            if allowed is not None and c["room"] not in allowed:
                bad.append((_lbl(c["trid"]),c["pnaz"][:24],c["room"],"povoleno:"+",".join(allowed)))
        for i in range(c["L"]):
            h=c["h1"]+i
            for t in c["teachers"]: tocc[(t,c["d"],h)].append(1)
            if c.get("room"): rocc[(c["room"],c["d"],h)].append(1)
            for sc in subs(c["trid"],c["obor"]): cocc[(sc,c["d"],h)].append(1)
    tc=sum(1 for v in tocc.values() if len(v)>1)
    rc=sum(1 for v in rocc.values() if len(v)>1)
    cc=sum(1 for v in cocc.values() if len(v)>1)
    print(f"{jf}: buněk {len(cells)} | konflikty uč/učeb/tř {tc}/{rc}/{cc} | učebna MIMO dokument: {len(bad)}")
    for b in bad[:10]: print("   ✗",b)
verify("gen_pred_v19_A.json")
verify("gen_pred_v19_B.json")
