...
def rooms_of(naz,mist):
    ...
    if "prostorový design" in p: return ["BD4","BD1"]
    if "game" in p: return ["Kn","IT2"]
    ...
    return []
def end_cap(p):
    pl=p.lower(); return 7 if any(k in pl for k in ("český jazyk","matematika","fyzika","chemie","dějiny umění")) else 10
def spread(p):
    pl=p.lower(); return any(k in pl for k in ("matematika","fyzika","občanská nauka","dějiny umění","dějepis"))
def is_media(p):
...
def t_ok(uk,d,hh,L):
    ...
    if uk=="UXS9D":  # Švehlová — připnuté reálné sloty z loňska
        _alw={1:{4,5,8,9,10},3:{1,2,3,4,5,6,8,9,10},4:{1,2,3,7,8,9}}.get(d,set())
        if any(x not in _alw for x in range(hh,hh+L)): return False
...
for (trid,kp,skup,uk),hod in agg.items():
    ...
    wh=hod/2.0
    forceL = 3 if kp=="PT" else None
    Ls=[forceL] if forceL else blk(wh,art,md,sp)
    for L in Ls:
...
def subkeys(u):
    ...
    return [(t,u["skup"])]
TLIMIT={"UTS88":4,"UOS6U":4,"UZS4O":4,"UOS6X":3,"UVS8S":2}
def solve(group,tbusy,rbusy,tsec,tdays_prior):
    ...
    for ui,u in enumerate(group):
        ...
        if pl: out.append({"trid":u["trid"],"d":pl[0],"h1":pl[1],"L":u["L"],"teachers":u["teachers"],
