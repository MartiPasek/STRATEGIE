...
def subkeys(u):
    ...
    return [(t,u["skup"])]
TLIMIT={"UTS88":4,"UOS6U":4,"UZS4O":4,"UOS6X":3,"UVS8S":2,"UUS8D":4,"UPS70":3}
def solve(group,tbusy,rbusy,tsec,tdays_prior,hint=None):
    ...
    terms=[]
    for ui,u in enumerate(group):
        w=1000+8*u["L"]+(250 if any(t in WATCH for t in u["teachers"]) else 0)+(700 if "UNS6G" in u["teachers"] else 0)+(2600 if "UXS9D" in u["teachers"] else 0)+rr.randint(0,7)
