...
import re as _re
# ── Přesné učebny z "učebny pro předměty.docx" (Klárka) — per ročník+obor, 1.volba + náhrada ──
...
def room_doc(trid,naz):
    cl=_lbl(trid)
    if _re.match(r"^5[AB]$",naz) and cl=="3.GD": return ["BNA","BPG","BŠ"]  # GDN-G/I skupiny
    ab=_abbr_of(naz)
...
# VŠEOBECNÉ (akademické) — Klárka 23.6.: sázíme JEN odborné, všeobecné až naposled (další blok).
ODB_ONLY=True  # True = vynech všeobecné (placeholderem zůstanou sloty pro pozdější pass)
VSEOB_KW=["český jazyk","literatura","literární","matematik","fyzik","chemie","dějepis",
 "dějiny","historie","občansk","společenskovědní","základy spol","základy společ","zeměp",
...
def t_ok(uk,d,hh,L):
    ...
    if uk=="UNS6G":  # Vlková: ne čtvrtek + start>=2h
...
# Klárka 23.6.: jen odborné (velké ateliérové bloky první -> menší); všeobecné až další blok.
if ODB_ONLY:
    _before=len(units)
    units=[u for u in units if not is_vseob(u["pnaz"])]
    print("ODB_ONLY: vyhozeno vseobecnych:",_before-len(units),"zbyva odbornych:",len(units))
units.sort(key=lambda u:(-u["L"], 0 if u["art"] else 1))
# busy z jazyků + TV (lang3 = banded, varianta dle VARIANT; sedí na verzi v DB)
_VLI={"A":0,"B":1,"C":2,"D":3,"E":4,"F":5}.get(VARIANT,0)
lang=json.load(open("gen_lang3_out.json"))[_VLI]["cells"]
cbusy=set()
for c in lang:
...
def solve(group,tbusy,rbusy,tsec,tdays_prior,hint=None):
    ...
    # Klárka 23.6.: Vlková (UNS6G) ne STÁLE do večera = max 1 den s výukou v 9.-10. h (default)
    _ev=[]
    for _d in range(1,6):
    ...
    terms=[]
    for ui,u in enumerate(group):
        w=1000+60*u["L"]+(250 if any(t in WATCH for t in u["teachers"]) else 0)+(700 if "UNS6G" in u["teachers"] else 0)+(700 if "UXS9D" in u["teachers"] else 0)+rr.randint(0,7)
        for k,y in enumerate(yv[ui]):
            wk=w
            if VARIANT=="C":
                _d,_h,_r=cand[ui][k]; wk=w-4*(_h-1)   # C: zaplňuj shora (dřívější hodina = vyšší váha)
            terms.append(wk*y)
