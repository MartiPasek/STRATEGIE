...
# ===== DIAGNOSTIKA NEUMISTENYCH =====
print("\n===== DIAGNOSTIKA NEUMISTENYCH BLOKU =====")
# final busy z vysledku o (vse: phase1+phase2 odborne) + lang/TV (cbusy/tbusy0/rbusy0)
_FT=set(tbusy0); _FR=set(rbusy0)
for _c in o:
    for _h in range(_c["h1"],_c["h1"]+_c["L"]):
        for _t in _c["teachers"]: _FT.add((_t,_c["d"],_h))
        if _c.get("room"): _FR.add((_c["room"],_c["d"],_h))
def _slots(u, tb, rb, ignore_odb_room=False):
    res=[]
    for d in range(1,6):
        for h in range(1,u["cap"]-u["L"]+2):
            e=h+u["L"]-1
            ...
            hrs=range(h,e+1)
            if any((u["trid"],d,x) in cbusy for x in hrs): continue
            if any((t,d,x) in tb for t in u["teachers"] for x in hrs): continue
            if any(not t_ok(t,d,h,u["L"]) for t in u["teachers"]): continue
            rl=u["rooms"] if u["art"] else [""]
            okrooms=[r for r in rl if not (r and any((r,d,x) in rb for x in hrs))]
            if okrooms: res.append((d,h,tuple(okrooms)))
    return res
_bykey={}
for u in units: _bykey[(u["trid"],u["pnaz"],u["teachers"][0])]=u
for (t,p,L,uk) in sorted(un):
    u=_bykey.get((t,p,uk))
    if not u: print("  ?? nenalezen",t,p,uk); continue
    rooms = u["rooms"] if u["art"] else ["(kmen)"]
    langonly=_slots(u, tbusy0, rbusy0)      # jen proti jazykum/TV
    final   =_slots(u, _FT, _FR)            # proti vsemu
    cls={0:"HARD(jazyky/TV blokuji)"}.get(len(langonly)) if len(langonly)==0 else ("KONTENCE(odborne zabraly)" if len(final)==0 else "SOLVER(volno je, jen nestihl)")
    print("  %-3s %-26s L%d uk=%s rooms=%s"%(t,p[:26],L,uk,",".join(rooms[:6])))
    print("       volnych oken: jen_proti_jaz=%d  proti_vsemu=%d  => %s"%(len(langonly),len(final),cls))
    if 0<len(langonly)<=8:
        print("       okna(d,h,rooms) proti jazykum:", langonly[:8])
