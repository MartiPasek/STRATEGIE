    ...
    for cc,pairs in COH.items():
        for mains,subs in pairs:
            for d in range(1,6):
                for x in range(1,11):
                    _mv=[]; _sv=[]
                    for _ln in mains: _mv+=claneSpec.get((cc,_ln,d,x),[])
                    for _ln in subs: _sv+=claneSpec.get((cc,_ln,d,x),[])
                    mo=None; so=None
                    if _mv:
                        mo=m.NewBoolVar("M_%s_%d_%d"%(cc,d,x))
                        for _v in _mv: m.Add(mo>=_v)
                        m.Add(mo<=sum(_mv))
                    if _sv:
                        so=m.NewBoolVar("S_%s_%d_%d"%(cc,d,x))
                        for _v in _sv: m.Add(so>=_v)
                        m.Add(so<=sum(_sv))
...
_b=json.load(open("busy_phase1.json"))
_tb=set(tbusy0)|set(tuple(x) for x in _b["tb"])
_rb=set(rbusy0)|set(tuple(x) for x in _b["rb"])
o=[]; un=[]; assign=[None]*len(units)
for _cc in ["26","1W","2B","2F","2A"]:
    _gu=[u for u in units if u["trid"]==_cc]
    if not _gu: continue
    _t={"26":8.0,"1W":8.0,"2B":7.0,"2F":7.0,"2A":6.0}.get(_cc,5.0)
    ...
    print("  %s: %d/%d"%(_cc,len(_oc),len(_gu)))
_p1=json.load(open("cells_phase1.json"))["cells"]
o=o+_p1
...
# ===== GREEDY POST-FILL (lane-aware) na vysledek o/un =====
print("\n===== GREEDY POST-FILL =====")
# busy z langs/TV
_GT=set(tbusy0); _GR=set(rbusy0); _GL=set()  # (trid,lane,d,h)
def _occ_cell(c):
    u_subs=None
    # lanes for this cell
    t=c["trid"]; sk=c.get("obor")
    if sk==WHOLE.get(t): lanes=[(t,ln) for ln in LANES.get(t,[sk])]
    else: lanes=[(t,sk)]
    for h in range(c["h1"],c["h1"]+c["L"]):
        for tt in c["teachers"]: _GT.add((tt,c["d"],h))
        if c.get("room"): _GR.add((c["room"],c["d"],h))
        for ln in lanes: _GL.add((ln[0],ln[1],c["d"],h))
for c in o: _occ_cell(c)
def _fit(u):
    subs=subkeys(u)
    for d in range(1,6):
        for h in range(1,u["cap"]-u["L"]+2):
            e=h+u["L"]-1
            if e>u["cap"] or (d==5 and e>7): continue
            hrs=list(range(h,e+1))
            if any((u["trid"],d,x) in cbusy for x in hrs): continue
            if any((sc[0],sc[1],d,x) in _GL for sc in subs for x in hrs): continue
            if any((tt,d,x) in _GT for tt in u["teachers"] for x in hrs): continue
            if any(not t_ok(tt,d,h,u["L"]) for tt in u["teachers"]): continue
            rl=u["rooms"] if u["art"] else [""]
            for r in rl:
                if r and any((r,d,x) in _GR for x in hrs): continue
                return (d,h,r)
    return None
_bykey={}
for u in units: _bykey[(u["trid"],u["pnaz"],u["teachers"][0])]=u
# phase1 unplaced (Tesliuk Motion) taky zkus
_allun=list(un)
gained=0
for (t,p,L,uk) in sorted(_allun, key=lambda z:(-z[2])):
    u=_bykey.get((t,p,uk))
    if not u: continue
    pl=_fit(u)
    if pl:
        d,h,r=pl
        nc={"trid":t,"d":d,"h1":h,"L":L,"teachers":u["teachers"],"pnaz":p,"room":r,"obor":u["skup"]}
        o.append(nc); _occ_cell(nc); gained+=1
        print("  + DOPLNENO %-3s %-24s d%d h%d-%d %s"%(t,p[:24],d,h,h+L-1,r or "(kmen)"))
    else:
        print("  - nelze    %-3s %-24s L%d %s"%(t,p[:24],L,uk))
tot=sum(u["L"] for u in units); pl2=sum(c["L"] for c in o)
print("PO FILLU: bunek %d, hodin %d/%d = %d%%, doplneno %d blocku"%(len(o),pl2,tot,round(100*pl2/tot),gained))
json.dump({"cells":o,"un":[x for x in un if not any(c["trid"]==x[0] and c["pnaz"]==x[1] and c["teachers"][0]==x[3] for c in o[-gained:] )],"seed":SEED,"variant":VARIANT},open("gen_odb_FILL_%s_%d.json"%(VARIANT,SEED),"w"))
print("-> gen_odb_FILL_%s_%d.json"%(VARIANT,SEED))
