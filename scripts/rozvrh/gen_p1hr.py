...
for (trid,kp,skup,uk),hod in agg.items():
    ...
    _pp=naz.lower(); _EXTRA=[]
    if any(k in _pp for k in ("3d animace","audiovizu","game","motion","web design","webdesign","multimedialni apl","multimediální apl","vizualizace","3d animace")) or _pp.startswith("3d") or ("animace" in _pp):
        _EXTRA=["IT2","MM","BŠ","BNA","BPG"]
    elif "prostorový design" in _pp:
        _EXTRA=["BD4","BD1","BA"]
    elif "figurální" in _pp or "výtvarná příprava" in _pp or "výtvarná tvorba" in _pp:
        _EXTRA=["BA","BK","BD1"]
    if rms and _EXTRA:
        for _r in _EXTRA:
            if _r not in rms: rms=rms+[_r]
...
def solve(group,tbusy,rbusy,tsec,tdays_prior,hint=None):
    ...
    for ui,u in enumerate(group):
        ...
        if ys: (m.Add(sum(ys)==1) if "UNS6G" in u["teachers"] else m.Add(sum(ys)<=1))
...
_g=[u for u in units if u["trid"] in ("1U","25","2E")]
o,un,assign=solve(_g,set(tbusy0),set(rbusy0),SEC,{})
_tb=[]; _rb=[]
for _c in o:
    for _h in range(_c["h1"],_c["h1"]+_c["L"]):
        for _t in _c["teachers"]: _tb.append([_t,_c["d"],_h])
        if _c["room"]: _rb.append([_c["room"],_c["d"],_h])
json.dump({"tb":_tb,"rb":_rb}, open("busy_phase1.json","w"))
json.dump({"cells":o}, open("cells_phase1.json","w"))
print("PHASE1 GDN:",len(o),"/",len(_g),"neum",len(un))
