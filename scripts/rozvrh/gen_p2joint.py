...
def _build_hint(group):
    try: best=json.load(open("gen_odb_BEST.json"))["cells"]
    except Exception: return None
    pool=defaultdict(list); cls=set(u["trid"] for u in group)
    for c in best:
        if c["trid"] in cls:
            pool[(c["trid"],c["pnaz"],c["L"])].append((c["d"],c["h1"],c.get("room","") or ""))
    h=[]
    for u in group:
        k=(u["trid"],u["pnaz"],u["L"])
        h.append(pool[k].pop(0) if pool.get(k) else None)
    return h
o=[]; un=[]
_classes=["26","1W","2B","2F","2A"]
_gu=[u for u in units if u["trid"] in _classes]
_hint=_build_hint(_gu)
print("JOINT start: %d jednotek 5 trid, hint=%d, cas=%ss seed=%d"%(len(_gu),sum(1 for x in (_hint or []) if x),SEC,SEED),flush=True)
_oc,_uc,_ac=solve(_gu,_tb,_rb,SEC,{},hint=_hint)
o=_oc; un=_uc
print("JOINT done: %d/%d jednotek umisteno (5 spornych trid)"%(len(_oc),len(_gu)),flush=True)
