...
def _abbr_of(naz):
    ...
    return None
# Klárka e-mail 29.6.2026: ZÁVAZNÉ učebny pro 7 dříve neumístěných bloků (per třída+předmět).
# Má přednost před room_doc/rooms_of. (label, abbr) -> seznam povolených učeben.
KLARKA_ROOMS={
  ("4.GD","GDN"):["BNA","BPG","BŠ"],   # grafický design a navrhování (Pejřimovská) + BŠ navíc
  ("4.GD","Te"):["BA"],                # technologie (Němejc) — pouze BA
  ("4.MI","A3D"):["MM","IT2"],         # 3D animace — pouze MM, IT2
  ("1.MI","A3D"):["MM","IT2"],         # 3D animace — pouze MM, IT2
  ("1.MI","A"):["MM","IT2"],           # animace — pouze MM, IT2
  ("1.MI","PT"):["BD1"],               # prostorová tvorba — pouze BD1
}
def room_doc(trid,naz):
    cl=_lbl(trid)
    _ab0=_abbr_of(naz)
    if _ab0 and (cl,_ab0) in KLARKA_ROOMS: return list(KLARKA_ROOMS[(cl,_ab0)])
...
def t_ok(uk,d,hh,L):
    ...
    if uk=="UUS8I" and d not in (3,4): return False  # Klarka 29.6: Stichenwirthova jen st NEBO ct (K5)
    return True
raw=[]
...
for _cc in ["1U","26","1W","2B","2F"]:
    _gu=[u for u in units if u["trid"]==_cc]
    if not _gu: continue
    _t={"1U":10.0,"26":8.0,"1W":8.0,"2B":7.0,"2F":7.0}.get(_cc,5.0)
