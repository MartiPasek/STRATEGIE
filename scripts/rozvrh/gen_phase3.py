...
units=[u for u in units if not any(is_atel(r) or r in _LAB for r in u["rooms"])]
print("kmenovych jednotek:",len(units))
...
_at=json.load(open("gen_odb_A_1.json"))["cells"]
for _c in _at:
    for _h in range(_c["h1"],_c["h1"]+_c["L"]):
        cbusy.add((_c["trid"],_c["d"],_h))
        for _t in _c["teachers"]: tbusy0.add((_t,_c["d"],_h))
        if _c["room"]: rbusy0.add((_c["room"],_c["d"],_h))
print("po atelieru: cbusy",len(cbusy))
_tb=set(tbusy0); _rb=set(rbusy0)
o=[]; un=[]; assign=[None]*len(units)
for _cc in ["1U","25","26","1W","2A","2B","2F","2E"]:
    _gu=[u for u in units if u["trid"]==_cc]
    if not _gu: continue
    _oc,_uc,_ac=solve(_gu,_tb,_rb,4.8,{})
    o+=_oc; un+=_uc
    for _c in _oc:
        for _h in range(_c["h1"],_c["h1"]+_c["L"]):
            for _t in _c["teachers"]: _tb.add((_t,_c["d"],_h))
            if _c["room"]: _rb.add((_c["room"],_c["d"],_h))
    print("  %s kmen: %d/%d"%(_cc,len(_oc),len(_gu)))
o=o+_at
