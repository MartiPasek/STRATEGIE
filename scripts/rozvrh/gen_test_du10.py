...
def end_cap(p):
    pl=p.lower()
    if any(k in pl for k in ("český jazyk","matematika")): return 7
...
_lang=json.load(open("gen_lang3_out.json"))[_VLI]["cells"]
cbusy=set(); tbusy0=set(); rbusy0=set()
for _c in _lang:
    for _h in range(_c["h"], _c["h"]+_c.get("L",1)):
        if _c["c"] in GDMI: cbusy.add((_c["c"],_c["d"],_h))
        if _c.get("uk"): tbusy0.add((_c["uk"],_c["d"],_h))
_tv=json.load(open("gen_tv_week.json"))["cells"]
for _c in _tv:
    for _h in (_c["h1"],_c["h2"]):
        if _c["c"] in GDMI: cbusy.add((_c["c"],_c["d"],_h))
        tbusy0.add((_c["uk"],_c["d"],_h))
def subkeys(u):
    t=u["trid"]
...
def solve(group,tbusy,rbusy,tsec,tdays_prior,hint=None):
    ...
    terms=[]
    for ui,u in enumerate(group):
        w=1000+5*u["L"]+(9000 if u["skup"]==WHOLE.get(u["trid"]) else 0)+(250 if any(t in WATCH for t in u["teachers"]) else 0)+(700 if "UNS6G" in u["teachers"] else 0)+(700 if "UXS9D" in u["teachers"] else 0)+rr.randint(0,7)
...
_g=[u for u in units if u["trid"] in ("1U","25")]
...
print("PHASE1 GDN 1U+25:",len(o),"/",len(_g),"neum",len(un))
