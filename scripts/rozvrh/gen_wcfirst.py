...
def _isatel(u): return any(is_atel(r) or r in _LAB for r in u["rooms"])
_tb=set(tbusy0); _rb=set(rbusy0); o=[]; un=[]
for _cc in ("1U","25"):
    _km=[u for u in units if u["trid"]==_cc and not _isatel(u)]
    _at=[u for u in units if u["trid"]==_cc and _isatel(u)]
    # 1) kmenove (Dejiny umeni) - rezervuj
    _o1,_u1,_a1=solve(_km,_tb,_rb,6.0,{})
    for _c in _o1:
        for _h in range(_c["h1"],_c["h1"]+_c["L"]):
            cbusy.add((_c["trid"],_c["d"],_h))
            for _t in _c["teachers"]: _tb.add((_t,_c["d"],_h))
            if _c["room"]: _rb.add((_c["room"],_c["d"],_h))
    # 2) atelier kolem
    _o2,_u2,_a2=solve(_at,_tb,_rb,14.0,{})
    for _c in _o2:
        for _h in range(_c["h1"],_c["h1"]+_c["L"]):
            for _t in _c["teachers"]: _tb.add((_t,_c["d"],_h))
            if _c["room"]: _rb.add((_c["room"],_c["d"],_h))
    o+=_o1+_o2; un+=_u1+_u2
    print("  %s: kmen %d/%d, atelier %d/%d"%(_cc,len(_o1),len(_km),len(_o2),len(_at)))
assign=[None]*len(units)
_tb=[]; _rb=[]
for _c in o:
    for _h in range(_c["h1"],_c["h1"]+_c["L"]):
