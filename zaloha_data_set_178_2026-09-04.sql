 ...
 WITH wd AS (
   ...
   SELECT e.employee_id, em.full_name, em.cislo_zam::text AS cislo_zam,
          et.code AS typ_code, et.label AS typ_label, e.entry_date AS d,
   ...
   SELECT a.*, (wd.rn - row_number() OVER (PARTITION BY a.employee_id,a.typ_code,date_trunc('month',a.d),a.hod,a.zad_id ORDER BY a.d)) AS grp
   FROM a_days a LEFT JOIN wd ON wd.day=a.d
 ), a_period AS (
   SELECT employee_id, max(full_name) full_name, max(cislo_zam) cislo_zam,
          typ_code, max(typ_label) typ_label,
          ...
          max(zmeneno) zmeneno
   FROM a_isl GROUP BY employee_id, typ_code, date_trunc('month', d), hod, zad_id, grp
 ), b_period AS (
   ...
   SELECT employee_id,full_name,cislo_zam,typ_code,typ_label,odd,dokon,odd_ts,dokon_ts,pocet_dni,hodin_den,fond_den,dnu_poctem,schvaleno,created_at,src,created_by_id,zampozn,vedpozn,ids,radek_typ,autor_id,odkud,schvalil_id,schvalil_kdy,zmenil_id,zmeneno FROM a_period
   UNION ALL
   SELECT employee_id,full_name,cislo_zam,typ_code,typ_label,odd,dokon,odd_ts,dokon_ts,pocet_dni,hodin_den,fond_den,dnu_poctem,schvaleno,created_at,src,created_by_id,zampozn,vedpozn,ids,radek_typ,autor_id,odkud,schvalil_id,schvalil_kdy,zmenil_id,zmeneno FROM b_period
 ), dedup AS (
   SELECT *, row_number() OVER (
     PARTITION BY employee_id, typ_code, odd, dokon
